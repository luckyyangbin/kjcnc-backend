			...
			<a-form @submit="onSubmit" :form="form">
				<a-tabs size="large" :tabBarStyle="{textAlign: 'center'}" style="padding: 0 2px;">
					<a-tab-pane tab="邮箱登录" key="1">
						...
						<a-form-item>
							<a-input size="large" placeholder="888888" autocomplete="autocomplete" type="password" v-decorator="['password', {rules: [{ required: true, message: '请输入密码', whitespace: true}]}]">
				...
				<div>
					...
					<router-link style="float: left" :to="{path:'/'}">注册账户</router-link>
			...
			onSubmit(e) {
				...
				this.form.validateFields((err) => {
					if (!err) {
						...
						console.log(this.login(name, password))
					}
				})
			},
			login(name, password) {
				this.$cloudbase
					...
					.then((loginState) => {
						...
						setAuthorization({
							token: loginState.user._cache.keys.accessTokenExpireKey,
